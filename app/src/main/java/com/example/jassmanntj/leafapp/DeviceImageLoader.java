...
public class DeviceImageLoader {
	...
	public void loadImage(int channels, int width, int height, InputStream ims1, InputStream ims2) throws IOException {
        ...
        for(int channel = 0; channel < 3; channel++) {
            DenseDoubleMatrix2D feature = (DenseDoubleMatrix2D) d.subMatrix(images, 0, 0, height * width * channel, height*width * channel + height*width - 1);
            feature = reshape(feature, height, width);
            Log.d("INZZZ", "L"+feature.toString());
        }
        DenseDoubleMatrix2D i2 = normalizeData(images);
        for(int channel = 0; channel < 3; channel++) {
            DenseDoubleMatrix2D feature = (DenseDoubleMatrix2D) d.subMatrix(i2, 0, 0, height * width * channel, height*width * channel + height*width - 1);
            feature = reshape(feature, height, width);
            Log.d("ZZZ", feature.toString());
        }
	}
